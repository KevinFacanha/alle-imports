...
import { OAuthStateStore } from '../../../integrations/oauth/oauth-state.store.js';
...
describe('OAuthStateStore', () => {
  it('generates a secure state and an RFC 7636 S256 challenge', () => {
    ...
    const first = store.create('mercado-livre');
    const second = store.create('mercado-livre');
    ...
    assert.equal(
      store.consume(first.state, 'mercado-livre'),
      first.codeVerifier,
    );
    assert.throws(
      () => store.consume(first.state, 'mercado-livre'),
  ...
  it('rejects an invalid or expired state', () => {
    const store = new OAuthStateStore();
    const authorization = store.create('mercado-livre', 1_000);

    assert.throws(() =>
      store.consume('unknown', 'mercado-livre', 1_001),
    );
    assert.throws(() =>
      store.consume(
        authorization.state,
